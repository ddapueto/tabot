...
```
spec-01 (tests) ──→ spec-02 (CI/CD) ──→ spec-07 (deploy) ──→ spec-09 (WA real)
                                                          └──→ spec-08 (landing)
spec-06 (logging) ──→ spec-10 (monitoring) ←── spec-07

spec-03 (mobile), spec-04 (notifications), spec-05 (frontend): independientes
```

## Orden recomendado

```
Paralelo 1: spec-01 + spec-03 + spec-04 + spec-06
    ↓
Paralelo 2: spec-02 + spec-05
    ↓
spec-07 (deploy)
    ↓
